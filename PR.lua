--[[
    SCRIPT: XL3RL HUB - TEST ULTRA RÁPIDO
    FUNCIONES: 
      - NO-CLIP
      - AUTO-LÁSER (equipar + limpiar items en Heartbeat ultra rápido, disparo cada 0.2s)
]]
...
--                                   3. GUI BASE
-- =========================================================================================
pcall(function() if LP.PlayerGui:FindFirstChild("XL3RL_GUI_TEST") then LP.PlayerGui.XL3RL_GUI_TEST:Destroy() end end)
local Screen = make("ScreenGui", { Name = "XL3RL_GUI_TEST", ResetOnSpawn = false }, LP:WaitForChild("PlayerGui"))
...
local Main = make("Frame", {
    Size = UDim2.new(0, 160, 0, 150), Position = UDim2.new(0.5, -80, 0.5, -75),
...
local Layout = make("UIListLayout", {Padding = UDim.new(0,8), SortOrder = Enum.SortOrder.LayoutOrder}, Main)
...
-- Panel No-Clip
local NCPanel = make("Frame", {Size = UDim2.new(0, 160, 0, 100), Position = UDim2.new(0.5,-80,0.5,-50), BackgroundColor3 = Config.BackgroundColor, Visible=false}, Screen)
...
-- Panel Auto-Laser
local LaserPanel = make("Frame", {Size = UDim2.new(0, 160, 0, 100), Position = UDim2.new(0.5,-80,0.5,-50), BackgroundColor3 = Config.BackgroundColor, Visible=false}, Screen)
...
local function removeOtherTools()
    if LP.Character then
        for _,t in ipairs(LP.Character:GetChildren()) do
            if t:IsA("Tool") and not (t.Name=="Laser Cape" or string.find(t.Name:lower(),"laser")) then
...
local function makeMainButton(text, panel)
    local btn = make("TextButton", {
        ...
        TextColor3=Config.TextColor, BackgroundColor3=Config.MutedColor
    }, Main)
