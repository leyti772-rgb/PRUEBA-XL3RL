--[[
    Script: XL3RL (Versión Final Corregida)
    Descripción: Un GUI con No-Clip, múltiples ejecutores de scripts y función de minimizar/reabrir.
                 Corregido para que no se congele al cargar scripts con bucles infinitos.
]]
...
-- Frame principal que sirve como ventana
...
for _, btn in {NoClipButton, ChilliScriptButton, ServidoresScriptButton, AutoBaseButton} do
...
-- [[ FUNCIÓN MEJORADA PARA EJECUTAR SCRIPTS ]] --
local function executeScript(button, url)
    -- Inicia un nuevo hilo para que la GUI no se congele
    spawn(function()
        ...
        if success then
            ...
            warn("Error ejecutando script desde " .. url .. ": " .. tostring(result))
        end
    end)
end
