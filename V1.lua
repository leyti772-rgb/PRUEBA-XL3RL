--[[
    SCRIPT: XL3RL HUB - FUSIÓN FINAL (AutoLaser + Brainroot integración)
    AUTOR: Gemini + ChatGPT
    DESCRIPCIÓN:
        - Auto-Laser equipa/desequipa Laser Cape automáticamente.
        - Bloquea otras tools mientras está activo.
        - Re-equipamiento tras respawn si sigue activo.
        - Nuevo: Auto-Laser se desactiva automáticamente si el jugador agarra un "Brainroot".
]]
...
laserButton.MouseButton1Click:Connect(function()
    ...
    if autoLaser then
        ...
        task.spawn(function()
            while autoLaser do
                equipLaserCape()
                -- eliminar otras Tools para que no interfieran
                if LP.Character then
...
-- =========================================================================================
--        6. INTEGRACIÓN NUEVA: AUTO-DESACTIVAR AUTO-LASER SI AGARRA "BRAINROOT"
-- =========================================================================================
local function monitorBrainroot(char)
    char.ChildAdded:Connect(function(child)
        if child:IsA("Tool") and child.Name:lower():find("brainroot") then
            if autoLaser then
                autoLaser = false
                laserButton.Text, laserButton.BackgroundColor3 = "ACTIVAR", Color3.fromRGB(0,160,0)
                unequipLaserCape()
...
if LP.Character then monitorBrainroot(LP.Character) end
LP.CharacterAdded:Connect(monitorBrainroot)
