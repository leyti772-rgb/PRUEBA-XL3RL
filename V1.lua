--[[
    SCRIPT: XL3RL HUB - FUSIÓN FINAL (AutoLaser Forzado)
    DESCRIPCIÓN: Igual que la versión estable previa,
                 + Ahora mientras Auto-Laser esté activo, se fuerza cada 0.5s
                   el equipamiento de "Laser Cape" (para impedir equipar otra tool).
                 + Al desactivar, se devuelve al Backpack normalmente.
]]
...
local function getLaserTool()
    for _, parent in ipairs({LP.Backpack, LP.Character}) do
        if parent then
...
local function ensureLaserOwned(equipCallback)
    ...
    if tool then if equipCallback then equipCallback(tool) end return true end
...
local function equipLaserCape()
    local tool = getLaserTool()
    if tool then
        if LP.Character and tool.Parent ~= LP.Character then tool.Parent = LP.Character end
        return true
    else
        return ensureLaserOwned(function(t) if LP.Character then t.Parent = LP.Character end end)
    end
end
...
laserButton.MouseButton1Click:Connect(function()
    ...
    if autoLaser then
        ...
        task.spawn(function()
            while autoLaser do
                equipLaserCape() -- Forzar capa
                task.wait(0.5)   -- cada 0.5s
            end
        end)
    else
