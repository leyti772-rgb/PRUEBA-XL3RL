...
local function getLaserTool()
    ...
    if LP.Character then
        local t = LP.Character:FindFirstChild("Laser Cape")
        if t then return t end
    end
    return nil
end
...
-- Quitar otras tools
local function unequipAllExceptLaser()
    if LP.Character then
...
-- Comprar si falta
local function ensureLaserOwned(callback)
    local bp = LP:FindFirstChild("Backpack")
    if bp and bp:FindFirstChild("Laser Cape") then
        if callback then callback(bp:FindFirstChild("Laser Cape")) end
        return true
    end
    local ok = pcall(function()
        local t = ReplicatedStorage.Packages.Net:FindFirstChild("RF/CoinsShopService/RequestBuy"):InvokeServer("Laser Cape")
        if callback and t then callback(t) end
    end)
    return ok
end
...
local function equipLaserCape()
    ...
    if tool then
        ...
        unequipAllExceptLaser()
    else
        ensureLaserOwned(function(t)
            if LP.Character then
    ...
    laserConn = RunService.Heartbeat:Connect(function()
        for _,plr in ipairs(Players:GetPlayers()) do
            if plr ~= LP and plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") then
                ...
                local args = {
                    [1] = hrp.Position,
                    [2] = workspace:FindFirstChildWhichIsA("BasePart")
                }
...
-- Integrar al botón ya existente
laserButton.MouseButton1Click:Connect(function()
    autoLaser = not autoLaser
    if autoLaser then
        laserButton.Text = "DESACTIVAR AUTO-LASER"
        TweenService:Create(laserButton, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(170,20,20)}):Play()
        task.spawn(function()
            equipLaserCape()
        end)
        startAutoLaser()
        ...
        TweenService:Create(laserButton, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(0,160,0)}):Play()
        stopAutoLaser()
        unequipLaserCape()
    end
end)

-- Re-equipar al respawnear si estaba activo
