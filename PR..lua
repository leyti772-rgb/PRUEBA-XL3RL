--[[
    SCRIPT: XL3RL HUB - FUSIÓN FINAL ACTUALIZADO
    AUTOR: Gemini + ChatGPT
    DESCRIPCIÓN:
      - Incluye No-Clip y Auto-Laser.
      - Auto-Laser equipa capa automáticamente, desequipa al desactivar y tras respawn vuelve a equipar.
      - Elimina otros ítems mientras Auto-Laser está activo para que no se mezclen.
      - Apaga Auto-Laser automáticamente cuando se agarra un BRAINROOT.
      - Compatible con el nuevo RemoteEvent detectado por SimpleSpy.
]]
...
local NCPanel = make("Frame", {
    Size = UDim2.new(0, 160, 0, 100), Position = UDim2.new(0.5, -80, 0.5, -50),
    BackgroundColor3 = Config.BackgroundColor, Visible = false
}, Screen)
make("UICorner", {CornerRadius = UDim.new(0, 8)}, NCPanel)
local ncTitle = make("TextLabel", {Size = UDim2.new(1,0,0,30), Text = "NO-CLIP", TextColor3 = Config.MainColor, BackgroundTransparency = 1}, NCPanel)
local ncButton = make("TextButton", {Size = UDim2.new(1,-20,0,40), Position = UDim2.new(0,10,0,40), Text = "ACTIVAR", TextColor3 = Config.TextColor, BackgroundColor3 = Color3.fromRGB(0,160,0)}, NCPanel)
...
local laserTitle = make("TextLabel", {Size = UDim2.new(1,0,0,30), Text = "AUTO-LASER", TextColor3 = Config.MainColor, BackgroundTransparency = 1}, LaserPanel)
local laserButton = make("TextButton", {Size = UDim2.new(1,-20,0,40), Position = UDim2.new(0,10,0,40), Text = "ACTIVAR", TextColor3 = Config.TextColor, BackgroundColor3 = Color3.fromRGB(0,160,0)}, LaserPanel)
...
local function equipLaserCape()
    ...
    if not tool and BuyRF then pcall(function() BuyRF:InvokeServer("Laser Cape") end) task.wait(0.5) tool = getLaserTool() end
...
-- Loop para eliminar otros ítems
local function clearOtherTools()
    if LP.Character then
        for _,t in ipairs(LP.Character:GetChildren()) do
            if t:IsA("Tool") and t.Name ~= "Laser Cape" then t.Parent = LP.Backpack end
        end
    end
end
...
-- =========================================================================================
-- 7. DETECTAR GRAB BRAINROOT (Nuevo Remote)
-- =========================================================================================
local GrabRemote = ReplicatedStorage:FindFirstChild("RE/280b459b-b3c8-424e-9b0b-821d4a4dec11", true)
if GrabRemote then
...
local function makeMainButton(text, panel)
    local btn = make("TextButton", {
        ...
        TextColor3 = Config.TextColor, BackgroundColor3 = Color3.fromRGB(40,42,50)
    }, MainContent)
...
makeMainButton("NO-CLIP", NCPanel)
