...
function Library.UI:CreateActionButton(text, callback)
    local button = Library.Functions.CreateElement("TextButton", {
        ...
        LayoutOrder = #Library.UI.OptionsContainer:GetChildren() + 1
    ...
    local label = Library.Functions.CreateElement("TextLabel", {
        Size = UDim2.new(1, -10, 1, 0), Text = " " .. text, Font = Enum.Font.Code,
        TextSize = 13, TextColor3 = Library.Config.TextColor, BackgroundTransparency = 1,
        TextXAlignment = Enum.TextXAlignment.Left, Position = UDim2.new(0, 10, 0, 0)
    }, button)
    Library.Functions.CreateElement("TextLabel", {
        Name = "Icon", Size = UDim2.new(0, 20, 1, 0), Position = UDim2.new(0, 0, 0, 0),
        Text = ">", Font = Enum.Font.Code, TextSize = 13, TextColor3 = Library.Config.MainColor,
...
function Library.Features:ExecuteScript(button, urls)
    task.spawn(function()
        ...
        for i, url in ipairs((type(urls) == "table") and urls or {urls}) do
            ...
            if not s then Library:Notify("Error al cargar script " .. i, 2); warn("XL3RL: "..tostring(r)); break end
        end
        label.Text = originalText
        Library:Notify("Listo", 2)
    end)
end
...
function Library:Initialize()
    ...
    -- Actualiza el CanvasSize cuando cambie el contenido
    ...
    local openButtonTrigger = self.Functions.CreateElement("TextButton", { Size = UDim2.fromScale(1,1), Text = "", BackgroundTransparency = 1 }, openButton)
    ...
    -- Contenido scrollable + layout (evita panel vacío)
    ...
    local ncLayout = self.Functions.CreateElement("UIListLayout", {
    ...
    -- Toggle No-Clip
    local toggleFrame = self.Functions.CreateElement("Frame", {
        Size = UDim2.new(1, -10, 0, 40), BackgroundTransparency = 1
    }, ncContent)
    self.Functions.CreateElement("TextLabel", {
    ...
    local toggleButton = self.Functions.CreateElement("TextButton", {
        Size = UDim2.new(0, 45, 0, 22), Position = UDim2.new(1, -55, 0.5, -11),
        BackgroundColor3 = self.Config.BackgroundColor, Text = "", AutoButtonColor = false
    }, toggleFrame)
    self.Functions.CreateElement("UICorner", { CornerRadius = UDim.new(0, 4) }, toggleButton)
    self.Functions.CreateElement("UIStroke", { Color = self.Config.MainColor, Thickness = 1.5 }, toggleButton)
    local knob = self.Functions.CreateElement("Frame", {
        Size = UDim2.new(0, 16, 0, 16), Position = UDim2.new(0, 3, 0.5, -8),
        BackgroundColor3 = self.Config.MainColor
    }, toggleButton)
    self.Functions.CreateElement("UICorner", { CornerRadius = UDim.new(0, 2) }, knob)
    ...
    self.UI:CreateActionButton("Combo Scripts", function(b) self.Features:ExecuteScript(b, {
        "https://raw.githubusercontent.com/tienkhanh1/spicy/main/Chilli.lua",
        "https://gist.githubusercontent.com/UCT-hub/5b11d1b8ce08feb803861e0b79/raw/b2917b398d4b0cc80fb2aca73a3137ba494ebcf0/gistfile1.txt"
    }) end)
    self.UI:CreateActionButton("Cambiar Servidor", function(b) self.Features:ExecuteScript(b,
        "https://raw.githubusercontent.com/Gato-bytes/GatoHubOnTop/refs/heads/main/AutoJoin.lua") end)

    -- Forzar CanvasSize una vez al final
    task.wait()
    self.UI.OptionsContainer.CanvasSize = UDim2.new(0, 0, 0, uiLayout.AbsoluteContentSize.Y)
