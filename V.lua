--[[
    SCRIPT: XL3RL - V3 (Professional Redesign)
    AUTHOR: Gemini
    DESCRIPTION: Third version addressing all user feedback.
                 - Definitive fix for the empty No-Clip panel bug.
                 - Complete visual overhaul for a smaller, more professional, and minimalist UI.
                 - New button designs, new color scheme, and a custom-built menu icon.
]]
...
Library.Config = {
    Name = "XL3RL",
    MainColor = Color3.fromRGB(130, 80, 255),    -- Sophisticated Purple
    BackgroundColor = Color3.fromRGB(25, 26, 30), -- Dark, clean background
    MutedColor = Color3.fromRGB(50, 52, 60),      -- For inactive elements
...
function Library:Notify(message, duration)
    ...
    self.UI.StatusText.Text = "" -- Cleaner look
end
...
function Library.UI:CreateToggle(parent, text, callback)
    -- BUG FIX: The size is now correctly calculated to account for parent padding.
    local frame = Library.Functions.CreateElement("Frame", {
        Size = UDim2.new(1, -20, 0, 40), -- Corrected size
...
-- ... (El resto de las funciones de features se mantienen igual)
function Library.Features:ExecuteScript(button, urls)
    task.spawn(function()
        ...
        for i, url in ipairs(urlList) do
            Library:Notify("Executing " .. i .. "/" .. #urlList, 1)
...
function Library:Initialize()
    ...
    self.Functions.CreateElement("Frame", { -- Title underline
    ...
    -- Status text moved to the bottom
    ...
    self.UI.OptionsContainer = self.Functions.CreateElement("ScrollingFrame", {
        Size = UDim2.new(1, 0, 1, -75), -- Adjusted size
    ...
    openButton.Parent = screenGui -- Parent at the end
    
    -- Drag functionality
    self.Functions.MakeDraggable(mainFrame, mainFrame) -- Whole frame is draggable
    ...
    self.Functions.CreateElement("Frame", { -- Underline
