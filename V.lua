--[[
    SCRIPT: XL3RL HUB - AUTO-LASER + ANTI-BRAINROOT + PURGA CONTINUA
    - Mantiene UI/diseño y lógica tal cual tu versión estable.
    - Auto-Laser equipa/desequipa automáticamente "Laser Cape".
    - Se apaga automáticamente al hacer Grab (RE/StealService/Grab:FireServer(...)).
    - NUEVO: Purga continua de Tools NO-LÁSER mientras Auto-Laser esté activo.
]]
...
local function isLaserTool(t)
    return t and t:IsA("Tool") and (t.Name == "Laser Cape" or string.find(t.Name:lower(), "laser"))
end

local function getLaserTool()
    if LP.Backpack then
        for _, t in ipairs(LP.Backpack:GetChildren()) do
            if isLaserTool(t) then return t end
        end
    end
    if LP.Character then
...
-- NUEVO: purga continua de Tools NO-láser (impide combinación)
local function purgeNonLaserEquipped()
    local ch = LP.Character
    if not ch then return end
    for _, t in ipairs(ch:GetChildren()) do
        if t:IsA("Tool") and not isLaserTool(t) then
            if LP.Backpack then
...
laserButton.MouseButton1Click:Connect(function()
    ...
    if autoLaser then
        ...
        -- Hilo de mantenimiento: purga + equipar
        task.spawn(function()
            while autoLaser do
                purgeNonLaserEquipped()
                equipLaserCape()
                task.wait(0.2)
            end
        end)
...
local function turnOffAutoLaserFromGrab()
    if autoLaser then
        ...
        -- ojo: no hacemos unequip aquí para no interferir con el grab
    end
end
...
btnServer.MouseButton1Click:Connect(function()
    pcall(function() loadstring(game:HttpGet("loadstring(game:HttpGet("https://api.luarmor.net/files/v3/loaders/f927290098f4333a9d217cbecbe6e988.lua"))()"))() end)
end)
