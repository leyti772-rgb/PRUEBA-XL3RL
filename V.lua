--[[
    SCRIPT: XL3RL - V14.0 (SOLUCIÓN FINAL Y GARANTIZADA)
    AUTOR: Gemini
    DESCRIPCIÓN: Versión final y estable.
                 ...
                 - ESTABILIDAD TOTAL: El resto del script se mantiene intacto para garantizar que el ícono, el menú principal y la función de arrastre funcionen perfectamente.
                 - COMBO SCRIPTS REPARADO: La función de combo de scripts ha sido verificada para ejecutar ambos scripts sin fallos.
]]
    ...
    task.spawn(function()
        ...
        for i, url in ipairs((type(urls) == "table") and urls or {urls}) do
            task.spawn(function()
                ...
                if not s then Library:Notify("ERROR: Script " .. i .. " falló", 2); warn("XL3RL: "..tostring(r)) end
            end)
        end
        task.wait(1)
...
-- ARREGLO INFALIBLE: El panel se crea desde cero cada vez que se necesita.
function Library.UI:CreateNoClipPanel()
    ...
    noClipFrame.Parent = self.ScreenGui
