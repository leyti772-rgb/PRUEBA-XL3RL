--[[
    SCRIPT: XL3RL - V4.2 (EDICIÓN ULTRA-COMPACTA)
    AUTOR: Gemini
    DESCRIPCIÓN: Versión final enfocada en tamaño mínimo y visibilidad absoluta.
                 - Menús reducidos a su tamaño más pequeño y funcional.
                 - Arreglo definitivo de visibilidad en panel No-Clip: fondo 100% opaco.
                 - Eliminado el botón "REINICIAR PERSONAJE".
]]
...
Library.Config = {
    ...
    SolidBackgroundColor = Color3.fromRGB(25, 27, 35), -- Color sólido para visibilidad
...
function Library.UI:CreateActionButton(text, callback)
    local button = Library.Functions.CreateElement("TextButton", {
        Size = UDim2.new(1, -16, 0, 32), -- Más pequeño
        Text = "",
...
function Library:Initialize()
    ...
    -- CAMBIO: Menú principal ultra-compacto
    ...
    -- CAMBIO: Panel No-Clip ultra-compacto y 100% opaco para máxima visibilidad
    local noClipFrame = self.Functions.CreateElement("Frame", {
        Name = "NoClipFrame",
        Size = UDim2.new(0, 200, 0, 100),
        Position = UDim2.new(0.5, -100, 0.5, -50),
        BackgroundColor3 = self.Config.SolidBackgroundColor, -- FONDO SÓLIDO
        BackgroundTransparency = 0, -- CERO TRANSPARENCIA
        BorderSizePixel = 0,
        Visible = false,
        ZIndex = 50 -- ZIndex alto para que esté por encima de todo
    }, screenGui)
    ...
    self.Functions.CreateElement("UIListLayout", { Padding = UDim.new(0, 5), HorizontalAlignment = Enum.HorizontalAlignment.Center, Parent = ncContent })
    
    self.UI:CreateToggle(ncContent, "ACTIVAR", function(state) self.Features:NoClip(state) end)
    ...
    self:Notify("XL3RL V4.2 [COMPACTO] CARGADO", 3)
end
