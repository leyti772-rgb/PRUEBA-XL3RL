...
-- Altura ajustada para 4 botones + título (compacto)
...
-- Contenido con padding y layout
...
--==================== Panel AUTO-LASER (compacto + sin hueco) ====================--
local LaserPanel = make("Frame", {
    Size = UDim2.new(0, 150, 0, 96),
    Position = UDim2.new(0.78, -75, 0.42, 0), -- debajo del No-Clip
    BackgroundColor3 = Color3.fromRGB(22,22,22),
    Visible = false
}, Screen)
...
local LContent = make("Frame", {
    Size = UDim2.fromScale(1,1),
    BackgroundTransparency = 1
}, LaserPanel)
local LPad = make("UIPadding", {
    PaddingTop = UDim.new(0, 6),
    PaddingBottom = UDim.new(0, 6),
    PaddingLeft = UDim.new(0, 8),
    PaddingRight = UDim.new(0, 8)
}, LContent)
local LLayout = make("UIListLayout", {
    Padding = UDim.new(0, 6),
    FillDirection = Enum.FillDirection.Vertical,
    HorizontalAlignment = Enum.HorizontalAlignment.Center,
    VerticalAlignment = Enum.VerticalAlignment.Top,
...
local LaserButton = make("TextButton", {
    ...
    TextColor3 = Color3.fromRGB(255,255,255),
    AutoButtonColor = false,
    BackgroundColor3 = Color3.fromRGB(170, 20, 20),
    LayoutOrder = 2
}, LContent)
...
LaserButton.MouseButton1Click:Connect(function()
    ...
    TweenService:Create(LaserButton, TweenInfo.new(0.2), {
        BackgroundColor3 = autoLaser and Color3.fromRGB(200, 40, 40) or Color3.fromRGB(170, 20, 20)
    }):Play()
end)
...
task.spawn(function()
    while task.wait(0.2) do
        if autoLaser and LaserRemote then
            ...
            if target then
                -- Disparo hacia el enemigo más cercano
...
btnCombo.MouseButton1Click:Connect(function()
    ...
    -- Links que PASASTE (combo scripts)
    pcall(function()
...
btnServer.MouseButton1Click:Connect(function()
    ...
    -- Link que PASASTE (cambiar servidor)
    pcall(function()
