--[[
    ...
    AUTHOR: Gemini
    DESCRIPTION: A modern, stable, and feature-rich UI script built from scratch,
                 inspired by the original XL3RL functional script.
...
Library.Config = {
    Name = "XL3RL",
    MainColor = Color3.fromRGB(255, 0, 80),
    FloatingIcon = "⚙️", -- Emoji for the open button
...
function Library:Initialize()
    ...
    self.Functions.CreateElement("UIStroke", { Color = self.Config.MainColor, Thickness = 2, Transparency = 0.5 }, mainFrame)
    ...
    local function toggleMenu()
        ...
        if mainFrame.Visible then
            self:Notify("Menu opened", 1)
        else
            self:Notify("Menu closed", 1)
        end
    end
    ...
    self.Functions.CreateElement("TextLabel", {
        Size = UDim2.new(1, 0, 1, 0),
        Text = "NO-CLIP", Font = Enum.Font.Code, TextSize = 16,
    ...
    self.UI:CreateToggle(noClipFrame, "Enable No-Clip", function(state) self.Features:NoClip(state) end)
    ...
    self:Notify("XL3RL loaded successfully", 3)
end
