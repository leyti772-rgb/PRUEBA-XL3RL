--[[
    SCRIPT: XL3RL - V3.1 (Rediseño Profesional - Español y Visibilidad Mejorada)
    AUTHOR: Gemini
    DESCRIPTION: Versión final con el diseño profesional solicitado, textos traducidos al español
                 y visibilidad mejorada para la opción de No-Clip.
]]
...
Library.Config = {
    Name = "XL3RL",
    MainColor = Color3.fromRGB(130, 80, 255),    -- Morado Sofisticado
    BackgroundColor = Color3.fromRGB(25, 26, 30), -- Fondo Oscuro Limpio
    MutedColor = Color3.fromRGB(65, 67, 75),      -- Para elementos inactivos (ligeramente más claro)
...
function Library:Notify(message, duration)
    ...
    self.UI.StatusText.Text = "" -- Aspecto más limpio
end
...
function Library.UI:CreateActionButton(text, callback)
    ...
    local corner = Library.Functions.CreateElement("UICorner", { CornerRadius = UDim.new(0, 6) }, button)
...
function Library.UI:CreateToggle(parent, text, callback)
    local frame = Library.Functions.CreateElement("Frame", {
        Size = UDim2.new(1, -20, 0, 40),
        BackgroundColor3 = Library.Config.MutedColor, -- Ligéramente más claro para mejor visibilidad
...
function Library.Features:ExecuteScript(button, urls)
    task.spawn(function()
        ...
        TweenService:Create(button, TweenInfo.new(0.2), { BackgroundTransparency = 1 }):Play()
        Library:Notify("Ejecución completa", 2)
    end)
end
...
function Library:Initialize()
    ...
    -- Main Frame (Rediseñado y más pequeño)
    ...
    self.Functions.CreateElement("UIStroke", { Color = self.Config.MainColor, Thickness = 1, Transparency = 0.5 }, mainFrame)
    ...
    -- Título Integrado (Sin barra de encabezado separada)
    ...
    self.Functions.CreateElement("Frame", { -- Subrayado del título
    ...
    -- Texto de estado movido a la parte inferior
    ...
    self.UI.OptionsContainer = self.Functions.CreateElement("ScrollingFrame", {
        Size = UDim2.new(1, 0, 1, -75), -- Tamaño ajustado
    ...
    -- Botón de Apertura (Ícono personalizado)
    ...
    -- Líneas del Ícono Personalizado
    for i = 1, 3 do
    ...
    openButton.Parent = screenGui -- Parent al final

    -- Funcionalidad de Arrastrar
    self.Functions.MakeDraggable(mainFrame, mainFrame) -- Toda la ventana es arrastrable
    ...
    -- Ventana de No-Clip (ARREGLADO & REDISEÑADO & TRADUCIDO)
    ...
    self.Functions.CreateElement("TextLabel", {
        Size = UDim2.new(1, 0, 0, 40), Text = "No-Clip",
        Font = Enum.Font.GothamBold, TextSize = 18, TextColor3 = self.Config.TextColor,
        BackgroundTransparency = 1
    }, noClipFrame)

    self.Functions.CreateElement("Frame", { -- Subrayado
    ...
    ncClose.MouseButton1Click:Connect(function()
        ...
        self:Notify("Panel No-Clip cerrado", 1)
    end)
    ...
    -- Crear Botones Principales (TRADUCIDOS)
    self.UI:CreateActionButton("NO-CLIP", function()
        noClipFrame.Visible = true
        self:Notify("Panel No-Clip abierto", 1)
    end)
    ...
    self.UI:CreateActionButton("SALTO DE SERVIDOR", function(button)
